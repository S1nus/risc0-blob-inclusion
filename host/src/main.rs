...
use methods::{BLOBRZ_ELF, BLOBRZ_ID};
...
fn main() -> anyhow::Result<()> {
    ...
    let header_bytes = std::fs::read("./host/header.dat")?;
    let dah = ExtendedHeader::decode_and_validate(&header_bytes)?;
    ...
    for proof in row_inclusion_proofs.iter() {
        println!(
            "proof valid: {}",
            proof.verify(dah.dah.hash().as_bytes().try_into()?)
        );
    }

    let blob_bytes = std::fs::read("./host/blob.dat")?;
    let mut blob = Blob::new(my_namespace, blob_bytes)?;
    // hardcoded because whoops
    blob.index = 8;

    let blob_size: u32 = (blob.data.len() / 512).try_into()?; // num shares
    println!("blob size: {}", blob_size);
    let square_size: u32 = row_roots.len().try_into()?;
    println!("Square size: {}", square_size);
    let blob_index: u32 = blob.index.try_into()?;
    ...
    let proofs_file = File::open("./host/proofs.json")?;
    let proofs: Vec<celestia_types::nmt::NamespaceProof> = serde_json::from_reader(proofs_file)?;
    ...
    let receipt = prover.prove(built_env, BLOBRZ_ELF)?;
    ...
    let output: bool = receipt.journal.decode()?;
    println!("result: {}", output);

    // The receipt was verified at the end of proving, but the below code is an
    // example of how someone else could verify this receipt.
    receipt.verify(BLOBRZ_ID)?;
